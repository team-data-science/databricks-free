...
This will open a new pipeline. Next type in the name `online_retail_pipeline`.
...
![Dropdown to select catalog and schema](/images/3-2/pipeline-catalog-and-schema.png)
...
![Start with an empty file button](/images/3-2/create-empty-file.png)

Now select a location and the language you want to use. 

For the langauge, we keep `Python`. For the folder path, you can keep it as it is or create its own directory for better structure. For this course, I will keep it simple and leave it at the default location. Click 'Select'.

![Filled out folder path and Python as language](/images/3-2/select-folder-path-and-language.png)

This will create a root folder of your pipeline with the name we provided `online_retail_pipeline` and the pipeline source folder with the name `transformations` and a single empty file `my_transformation.py`.
...
![Create folder button](/images/3-2/create-medallion-folders.png)

Name it `00_bronze`. 

Repeat this process for `01_silver` and `02_gold` respectively. 

![Medallion folders](/images/3-2/medallion-folders.png)

You could come up with different structures like more use-case centered folders but let's keep it simple.

Now remove the file `my_transformation.py` by right-clicking and selecting 'Move to Trash'. 

![Move to trash button](/images/3-2/move-to-trash.png)

Confirm the deletion and right click on the folder '00_bronze' and select 'Create file'.

Keep `Python` selected and then name it `ingest_orders`. For the 'Dataset type' you could select out of several options. 

![Different dataset types](/images/3-2/dataset-types-dialog.png)

But this time, we are going to do it from scratch. So keep `None selected` selected. Therefore, hit the 'Create' button.

![Create button to create pipeline source file](/images/3-2/create-ingest-orders.png)

This will open the pipeline editor and next to it a pipeline paragraph - also called DAG - which will show the lineage later. If you don't see pipeline graph, you can use the button on the right to open it:

![Pipeline graph](/images/3-2/pipeline-graph.png)
...
def bronze_raw_orders():
    return (spark.readStream
        ...
        .load("/Volumes/retail_pipeline/bronze/source_files")
        .withColumn("ingestion_timestamp", F.current_timestamp())
        .withColumn("source_file", F.col("_metadata.file_path"))
    )
```

First we import `pipelines` from `pyspark` as well as some helper functions and types. With materialized views where you process a lot of data at once, Declarative Pipelines can infer the schema. 

This, however, does not work when your data is processed as a stream datasets. Therefore, we define the schema ourselves.

The `StructField` takes the name of the column, which we keep untouched from the source to have a true bronze ingestion with the full schema from the source system. The we provide the data types and the third parameter is if the value is nullable.

Since we want to accept all values in Bronze and do our quality checks and cleaning later, we'll allow all values to be nullable.

Then we use `@dp.table` to define a streaming table and give it the name `bronze_orders`. Then we load the CSV, define it will have a header row, provide the schema which we defined before, load from the Volume we created before and add two audit columns that will help to know when the data was ingested and where it is coming from.

Run the pipeline by either using `CMD` + `Enter` or use the `Run Pipeline` button on the top.

![Run pipeline button](/images/3-2/run-pipeline-button.png)

The first run usually takes a while. But you can see the progress on the side and bottom of the editor.

![Pipeline progress](/images/3-2/pipeline-progress.png)

After around 1 minute you should see a successful run with information about the created table.

> [!NOTE] 
> In my tests, I needed to run the pipeline sometimes twice until the update was reflected even though both runs were successful. If that's the case, run it again.
...
```sql
SELECT * 
FROM retail_pipeline.bronze.bronze_orders VERSION AS OF 2
...
```sql
UPDATE retail_pipeline.bronze.bronze_orders
...
```sql
SELECT * 
FROM retail_pipeline.bronze.bronze_orders VERSION AS OF 3
WHERE InvoiceNo = '577696' 
  AND StockCode = '22197';
```
